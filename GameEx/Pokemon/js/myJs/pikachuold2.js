...
function getAudioName(file) {
    var pn = "audio" + file; var pid = document.getElementById(pn); if (!pid) { setAudioElement(pn, file); pid = document.getElementById(pn); }
    return pid;
}
...
function loadImage() { var list = new Array; var i; for (i = 1; i <= 36; i++)list.push(oStaticURL + "images/pieces" + i + ".png"); var t = new ImagePreloader(list, loadedAll); }
var startTime = (new Date()).getTime(); totalScorethis = 0; namesave = ''; timesavetwice = (new Date()).getTime(); var onlyyou = Math.floor((Math.random() * 1000000) + 100000); function printDate() { var dateobj = new Date(); var month = dateobj.getMonth(); var day = dateobj.getDate(); var year = dateobj.getFullYear(); if (day < 10) day = '0' + day; if (month < 10) month = '0' + month; var linkdate = day + "-" + month + "-" + year; var redirectlink = 'https://www.gamevui.com'; window.location = redirectlink; }
var MD5 = function (string) {
    ...
    function FF(a, b, c, d, x, s, ac) { a = AddUnsigned(a, AddUnsigned(AddUnsigned(F(b, c, d), x), ac)); return AddUnsigned(RotateLeft(a, s), b); }; function GG(a, b, c, d, x, s, ac) { a = AddUnsigned(a, AddUnsigned(AddUnsigned(G(b, c, d), x), ac)); return AddUnsigned(RotateLeft(a, s), b); }; function HH(a, b, c, d, x, s, ac) { a = AddUnsigned(a, AddUnsigned(AddUnsigned(H(b, c, d), x), ac)); return AddUnsigned(RotateLeft(a, s), b); }; function II(a, b, c, d, x, s, ac) { a = AddUnsigned(a, AddUnsigned(AddUnsigned(I(b, c, d), x), ac)); return AddUnsigned(RotateLeft(a, s), b); }; function ConvertToWordArray(string) {
        ...
        string = string.replace(/rn/g, "n"); var utftext = ""; for (var n = 0; n < string.length; n++) {
...
function loadXMLDoc() {
    var timenowsave = (new Date()).getTime(); secondelay = Math.round(((timenowsave - timesavetwice) / 1000)); if (secondelay < 60) { alert("Bạn đã chơi được " + secondelay + "/60 giây!\n\nHai lần gửi Điểm Cao cách nhau tối thiểu 60 giây!"); return; }
    else { timesavetwice = (new Date()).getTime(); }
    if (namesave.length > 1) { alert('Bạn sẽ lưu lại điểm cao này với tên: ' + namesave); }
    else { namesave = prompt("Nhập tên bạn muốn lưu điểm(Từ 1 tới 15 ký tự)", "TaiGamePikachu.Vn"); if (namesave == null && namesave == "") { loadXMLDoc(); return; } }
    var endTime = (new Date()).getTime(); endTime = Math.round(((endTime - startTime) / 1000)); var xmlhttp; if (window.XMLHttpRequest) { xmlhttp = new XMLHttpRequest(); }
    else { xmlhttp = new ActiveXObject("Microsoft.XMLHTTP"); }
    xmlhttp.onreadystatechange = function () { if (xmlhttp.readyState == 4 && xmlhttp.status == 200) { document.getElementById("myDiv").innerHTML = xmlhttp.responseText; } }
    var isscrore = totalScorethis; var hash = MD5('' + onlyyou + isscrore + startTime + endTime); var rsdone = xmlhttp.send();
}
...
var PieceWidth = 25; var PieceHeight = 37; function newPiece(board, col, row, value) {
    var div = document.createElement("div"); div.image = document.createElement("img"); div.setImage = function (imgIndex) { this.image.src = oStaticURL + "images/pieces" + imgIndex + ".png"; this.image.width = PieceWidth; this.image.height = PieceHeight; this.valueInMatrix = imgIndex; }
...
var BoardWidth = PieceWidth * 18; var BoardHeight = PieceHeight * 11; var checksound = true; var totalscore = 0; function newBoard() {
    var div = document.createElement("div"); div.lines = new Array; for (var i = 0; i < 3; i++) { div.lines[i] = document.createElement("div"); div.lines[i].style.position = "absolute"; div.lines[i].style.visibility = "hidden"; div.lines[i].style.backgroundColor = "red"; div.appendChild(div.lines[i]); }
    ...
    div.style.position = "relative"; div.style.width = BoardWidth + "px"; div.style.height = BoardHeight + "px"; div.setNewLevel = function (level) { this.level = level; this.arrValue = getNewMatrix(); this.isWaiting = false; repaint("level", this.level); repaint("blood", this.blood); repaint("score", totalscore); this.applyMatrix(); startCountDown(); }
    ...
    div.disp_saveScore = function () { showScore(totalScorethis); }
    div.createBigGame = function () {
        if (checksound) { getAudioName("sound4").play(); }
        this.level = 1; this.blood = 10; this.score = 0; totalscore = 0; this.setNewLevel(this.level); repaint("score", 0);
    }
    div.changesound = function () {
        var change_soundID = jh("#change_sound"); if (checksound) { checksound = false; change_soundID.val("Bật tiếng"); change_soundID.attr("class", "soundOff"); }
    ...
    div.onClickPiece = function (evt, iClick, jClick) {
        if (checksound) { getAudioName("sound2").play(); }
        var mybutton = getMouseButton(evt); if (mybutton == 0) {
            if (this.isWaiting) {
                this.isWaiting = false; var list = checkPath(this.arrValue, this.iFist, this.jFist, iClick, jClick); if (list != null) { this.arrValue[this.iFist][this.jFist] = 0; this.arrValue[iClick][jClick] = 0; this.arrPiece[this.iFist][this.jFist].setVisible(false); this.arrPiece[iClick][jClick].setVisible(false); this.drawPath(list); setTimeout("afterDrawPath()", 200); if (checksound) { getAudioName("sound5").play(); } } else {
                    if (checksound) { getAudioName("sound1").play(); }
                    this.arrPiece[this.iFist][this.jFist].setSelected(100);
                }
            } else { this.iFist = iClick; this.jFist = jClick; this.arrPiece[iClick][jClick].setSelected(50); this.isWaiting = true; }
        }
    }
...
function afterDrawPath() {
    for (var i = 0; i < 3; i++)
        el.lines[i].style.visibility = "hidden"; el.score += 10; if (el.level == 1) { totalScorethis += 10; repaint("score", el.score); } else { totalscore += 10; totalScorethis += 10; repaint("score", totalscore); }
    if (el.score == 720) { clearInterval(timeID); el.setNextLevel(); el.score = 0; return; }
    if (el.level > 1) { fixMatrix(el.arrValue, el.level); el.applyMatrix(); }
    if (isEnd(el.arrValue)) { el.blood--; repaint("blood", el.blood); if (el.blood == 0) { clearInterval(timeID); alert("Hết lượt đổi"); el.createBigGame(); if (checksound) { getAudioName("sound7").play(); } } else { alert("Không còn cặp nào có thể loại bỏ"); repairMatrix(el.arrValue); el.applyMatrix(); if (checksound) { getAudioName("sound8").play(); } } }
}
...
function newArray(col) {
    var a = new Array; for (var i = 0; i < col; i++)
...
function getNewMatrix() {
    var a = newArray(18); var i, j, k, t, remain, key; var stop; for (i = 0; i < 18; i++)
        for (j = 0; j < 11; j++)
            a[i][j] = 0; remain = 144; for (k = 1; k <= 36; k++) {
                for (t = 1; t <= 4; t++) {
                    key = getRandom(remain--) + 1; stop = false; for (i = 1; i <= 16; i++) {
                        if (stop)
...
function checkPath(a, i1, j1, i2, j2) {
    if (i1 == i2 && j1 == j2) return null; if (a[i1][j1] == 0 || a[i2][j2] == 0) return null; if (a[i1][j1] != a[i2][j2]) return null; var fist, last, i, j, t; var queue = new Array; var dad = newArray(18); var count = newArray(18); for (i = 0; i < 198; i++)queue[i] = new Point(0, 0); fist = 0; last = 0; queue[0].x = i1; queue[0].y = j1; for (i = 0; i < 18; i++)
        for (j = 0; j < 11; j++)
            dad[i][j] = new Point(-1, -1); dad[i1][j1].x = -2; count[i1][j1] = 0; var canGo = new Array; var p = new Array; var q = new Array; while (fist <= last) {
                i = queue[fist].x; j = queue[fist].y; fist++; for (t = 0; t < 4; t++) { canGo[t] = true; p[t] = i; q[t] = j; }
                do {
                    for (t = 0; t < 4; t++) {
                        if (canGo[t]) {
                            p[t] += UU[t]; q[t] += VV[t]; if (!myInside(p[t], q[t])) { canGo[t] = false; continue; }
                            if (p[t] == i2 && q[t] == j2) { dad[p[t]][q[t]].x = i; dad[p[t]][q[t]].y = j; return createArrayList(dad, i2, j2); }
                            if (a[p[t]][q[t]] > 0) { canGo[t] = false; continue; }
                            if (dad[p[t]][q[t]].x != -1) continue; if (count[i][j] == 2) continue; last++; queue[last].x = p[t]; queue[last].y = q[t]; dad[p[t]][q[t]].x = i; dad[p[t]][q[t]].y = j; count[p[t]][q[t]] = count[i][j] + 1;
                        }
                    }
...
function findTwin(a, i1, j1) {
    var fist, last, i, j, t; var queue = new Array; var cx = newArray(18); var count = newArray(18); for (i = 0; i < 198; i++)queue[i] = new Point(); fist = 0; last = 0; queue[0].x = i1; queue[0].y = j1; for (i = 0; i < 18; i++)
        for (j = 0; j < 11; j++)cx[i][j] = true; cx[i1][j1] = false; count[i1][j1] = 0; var canGo = new Array; var p = new Array; var q = new Array; while (fist <= last) {
            i = queue[fist].x; j = queue[fist].y; fist++; for (t = 0; t < 4; t++) { canGo[t] = true; p[t] = i; q[t] = j; }
            do {
                for (t = 0; t < 4; t++) {
                    if (canGo[t]) {
                        ...
                        if (!cx[p[t]][q[t]]) continue; if (count[i][j] == 2) continue; last++; queue[last].x = p[t]; queue[last].y = q[t]; cx[p[t]][q[t]] = false; count[p[t]][q[t]] = count[i][j] + 1;
                    }
                }
            } while (canGo[0] || canGo[1] || canGo[2] || canGo[3]);
        }
...
function findCheat(a) {
    var i, j; var b = new Array; var c = new Array; var k = 0; for (i = 1; i <= 16; i++)
        for (j = 1; j <= 9; j++)
...
function repairMatrix(a) {
    var b = new Array; var i, j, k = 0; for (i = 1; i <= 16; i++)
        for (j = 1; j <= 9; j++)
...
function fixZone(a, i1, j1, i2, j2, vector) {
    var i, j, p, q; var stop; do {
        stop = true; for (i = i1; i <= i2; i++)
            for (j = j1; j <= j2; j++)
                if (a[i][j] > 0) { p = i + UU[vector]; q = j + VV[vector]; if (p >= 1 && p <= 16 && q >= 1 && q <= 9) { if (a[p][q] == 0) { swap(a, i, j, p, q); stop = false; } } }
    } while (!stop);
}
function swap(a, i1, j1, i2, j2) { var tmp = a[i1][j1]; a[i1][j1] = a[i2][j2]; a[i2][j2] = tmp; }
function generateHV(n) {
    var a = new Array; var i, j, k, t; for (i = 0; i < n; i++)a[i] = n; j = n; for (i = 0; i < n; i++) {
        k = getRandom(j--) + 1; t = 0; while (k > 0) { if (a[t++] == n) k--; }
        a[t - 1] = i;
    }
    return a;
}
function mixArray(a, n) {
    var b = generateHV(n); var c = new Array; for (var i = 0; i < n; i++)
        c[i] = a[b[i]]; for (var i = 0; i < n; i++)
        a[i] = c[i];
}
function mixArray2(a, aa, n) {
    var b = generateHV(n); var c = new Array; var i; for (i = 0; i < n; i++)
        c[i] = a[b[i]]; for (i = 0; i < n; i++)
        a[i] = c[i]; for (i = 0; i < n; i++)
        c[i] = aa[b[i]]; for (i = 0; i < n; i++)
        aa[i] = c[i];
}
var timeID = 0; var elTimebar = document.getElementById("timebar"); var iffause = true; function startCountDown() { if (timeID > 0) clearInterval(timeID); elTimebar.tmp = 300; elTimebar.style.height = "300px"; timeID = setInterval("decTime()", 1250); }
function pausecomp(millis) {
    var date = new Date(); var curDate = null; do { curDate = new Date(); }
    while (curDate - date < millis);
}
function change_block() {
    if (el.blood == 0) { alert("Bạn đã hết lần đổi vị trí"); } else {
        if (checksound) { getAudioName("sound4").play(); }
        repairMatrix(el.arrValue); el.applyMatrix(); el.blood--; repaint("blood", el.blood);
    }
}
function decTime() {
    elTimebar.tmp--; elTimebar.style.height = elTimebar.tmp + "px"; if (elTimebar.tmp == 0) {
        if (el.checksound) { getAudioName("sound7").play(); }
        showScore(totalScorethis); clearInterval(timeID); setTimeout(function () { el.createBigGame(); }, 2000);
    }
}
function getMouseButton(evt) {
    if (navigator.appName == 'Microsoft Internet Explorer') { if (window.event) { var tmp = evt.button; if (tmp == 1) return 0; if (tmp == 4) return 1; if (tmp == 2) return 2; } else { return evt.button; } }
    else { return evt.button; }
}
function repaint(id, value) {
    var el = document.getElementById(id); if (el.hasChildNodes())
        el.removeChild(el.firstChild); el.appendChild(document.createTextNode(value));
}
function getRRR(p1, p2) {
    var x1, y1, x2, y2; if (p1.x < p2.x) { x1 = p1.x; x2 = p2.x; } else { x2 = p1.x; x1 = p2.x; }
